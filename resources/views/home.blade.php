    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
                ...
                map.on('click', 'fill-kec-layer', (e) => {
                    if (e.features && e.features.length > 0) {
                        ...
                        if (lastKecamatan !== namaKecamatan) {
                            ...
                            // Set filter desa
                            map.setFilter('batas-desa-layer', ['==', 'sub_district', namaKecamatan
                                .toUpperCase()
                            ]);
                            map.setFilter('fill-desa-layer', ['==', 'sub_district', namaKecamatan
                                .toUpperCase()
                            ]);
                            map.setFilter('label-desa', ['==', 'sub_district', namaKecamatan
                                .toUpperCase()
                            ]);
                            map.setFilter('hover-shadow', ['==', 'Name', namaKecamatan]);
                            map.setFilter('hover-shadow-desa', ['==', 'Name', namaKecamatan]);
                        }
                    }
                });
            ...
            function aggregateKomoditasByKecamatan(lokasi, selectedKomoditas) {
                ...
                lokasi.forEach(item => {
                    if (selectedKomoditas.includes(item.jenis_komoditas.toLowerCase())) {
            ...
            function renderMarkers() {
                ...
                renderTimeout = setTimeout(() => {
                    ...
                    const selectedKomoditas = Array.from(document.querySelectorAll(
                            '.komoditas-filter:checked'))
                    ...
                    lokasi.forEach(item => {
                        if (selectedKomoditas.includes(item.jenis_komoditas.toLowerCase())) {
                            ...
                            el.style.backgroundColor = getColorByKomoditas(item.jenis_komoditas);
                            ...
                            el.style.backgroundImage =
                                `url(${getIconByKomoditas(item.jenis_komoditas)})`;
                            el.style.backgroundSize = 'contain';
                            el.style.backgroundPosition = 'center';
                            el.style.backgroundRepeat = 'no-repeat';

                            // Ambil gambar pertama jika ada beberapa gambar
                            const gambar = Array.isArray(JSON.parse(item.gambar || '[]')) ? JSON
                                .parse(item.gambar)[0] : item.gambar;
                            const gambarUrl = gambar ? `apk_gis/public/storage/images/${gambar}` :
                            ...
                            const marker = new mapboxgl.Marker({
                                ...
                                .setPopup(new mapboxgl.Popup().setHTML(`
                                    <div style="max-width: 300px;">
                                        ...
                                        <!-- Tombol Detail -->
                                        <a href="/produk/${item.id}" class="btn-detail" style="display: inline-block; margin-top: 10px; padding: 5px 10px; background-color: #007bff; color: white; text-decoration: none; border-radius: 5px;">Lihat Detail</a>
                                    </div>
                                `))
                                .addTo(map);
            ...
            document.querySelectorAll('.komoditas-filter').forEach(cb => {
                cb.addEventListener('change', () => {
                    ...
                    const allChecked = Array.from(document.querySelectorAll('.komoditas-filter'))
                        .every(cb => cb.checked);

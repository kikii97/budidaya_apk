    ...
    <div class="content-wrapper">
        <section class="content pt-4">
            <div class="container-fluid">
                ...
                <form action="{{ route('admin.produk.update', $produk->id) }}" method="POST" enctype="multipart/form-data" class="mb-5">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="pembudidaya_id" name="pembudidaya_id" required
                            ...
                            <option value="" disabled {{ old('pembudidaya_id', $produk->pembudidaya_id) ? '' : 'selected' }}>Pilih Pembudidaya</option>
                            @foreach ($pembudidayas as $pembudidaya)
                                <option value="{{ $pembudidaya->id }}" {{ old('pembudidaya_id', $produk->pembudidaya_id) == $pembudidaya->id ? 'selected' : '' }}>{{ $pembudidaya->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="file" id="images" name="images[]" class="form-control form-control-sm" multiple accept="image/*">
                    ...
                    <div class="location-group">
                        <div class="row-layout">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control readonly-input" id="kecamatan" name="kecamatan" value="{{ old('kecamatan', $produk->kecamatan) }}" readonly required
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control readonly-input" id="desa" name="desa" value="{{ old('desa', $produk->desa) }}" readonly required
                                       oninvalid="this.setCustomValidity('Harap pilih lokasi pada peta.')"
                                       oninput="this.setCustomValidity('')">
                                @error('desa')
                                    <span class="text-danger">{{ $message }}</span>
                        ...
                        <div class="form-group mt-2">
                            ...
                            <input type="text" class="form-control" id="address" name="address" value="{{ old('address', $produk->alamat_lengkap) }}" required
                    ...
                    <div class="row-layout">
                        <div class="form-group">
                            ...
                            <input type="text" id="latitude" name="latitude" class="form-control readonly-input" value="{{ old('latitude', $produk->latitude) }}" readonly required
                                   oninvalid="this.setCustomValidity('Harap pilih lokasi pada peta.')"
                                   oninput="this.setCustomValidity('')">
                            @error('latitude')
                                <span class="text-danger">{{ $message }}</span>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="longitude" name="longitude" class="form-control readonly-input" value="{{ old('longitude', $produk->longitude) }}" readonly required
                                   oninvalid="this.setCustomValidity('Harap pilih lokasi pada peta.')"
                                   oninput="this.setCustomValidity('')">
                            @error('longitude')
                                <span class="text-danger">{{ $message }}</span>
                    ...
                    <div class="form-group mt-2">
                        ...
                        <select class="form-control" id="commodity_type" name="commodity_type" required
                                ...
                                oninput="this.setCustomValidity('')">
                            <option value="" disabled {{ old('commodity_type', $produk->jenis_komoditas) ? '' : 'selected' }}>Pilih Jenis Komoditas</option>
                    ...
                    <div class="form-group mt-2">
                        ...
                        <input type="text" class="form-control" name="specific_commodity_type" value="{{ old('specific_commodity_type', $produk->jenis_spesifik_komoditas) }}">
                    ...
                    <div class="row-layout">
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control" id="production_capacity" name="production_capacity" value="{{ old('production_capacity', $produk->kapasitas_produksi) }}">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="peak_production_period" value="{{ old('peak_production_period', $produk->masa_produksi_puncak) }}">
                    ...
                    <div class="row-layout">
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control" id="price_range_min" name="price_range_min" min="0" step="1" required placeholder="1000" value="{{ old('price_range_min', $produk->kisaran_harga_min) }}"
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control" id="price_range_max" name="price_range_max" min="0" step="1" required placeholder="3000" value="{{ old('price_range_max', $produk->kisaran_harga_max) }}"
                    ...
                    <div class="form-group mt-2">
                        ...
                        <input type="text" class="form-control" id="harvest_prediction" name="harvest_prediction" value="{{ old('harvest_prediction', $produk->prediksi_panen) }}" placeholder="Contoh: 21 April 2025" autocomplete="off">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const map = new mapboxgl.Map({
        ...
        center: [{{ old('longitude', $produk->longitude) ?: 108.3247 }}, {{ old('latitude', $produk->latitude) ?: -6.3265 }}],
        zoom: 14,
    ...
    map.on('load', () => {
        ...
        marker = new mapboxgl.Marker({ draggable: true, color: '#FF0000' })
            .setLngLat([{{ old('longitude', $produk->longitude) ?: 108.3247 }}, {{ old('latitude', $produk->latitude) ?: -6.3265 }}])
            .addTo(map);
        ...
        async function geocode(query) {
            const url = `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(query)}&format=json&addressdetails=1&countrycodes=ID&limit=5&viewbox=107.98,-6.00,108.50,-6.60&bounded=1`;
            try {
    ...
    const existingImages = Array.from(document.querySelectorAll('.delete-old-image')).map(btn => btn.getAttribute('data-filename'));
    ...
    function previewImages(event) {
        ...
        if (newFiles.length > 0) {
            newFiles.forEach(newFile => {
                if (newFile.type.startsWith('image/') && ['image/jpeg', 'image/png', 'image/jpg'].includes(newFile.type)) {
                    const exists = selectedFiles.some(file => file.name === newFile.name && file.size === newFile.size);
                    if (!exists) {
                        selectedFiles.push(newFile);
                    }
                } else {
                    alert(`File ${newFile.name} bukan gambar yang valid (hanya JPEG, PNG, atau JPG yang diperbolehkan).`);
                }
            });
            updateFileInput();
            renderPreview();
    ...
    document.querySelector('form').addEventListener('submit', (e) => {
        const currentExistingImages = Array.from(document.querySelectorAll('.delete-old-image')).map(btn => btn.getAttribute('data-filename'));
